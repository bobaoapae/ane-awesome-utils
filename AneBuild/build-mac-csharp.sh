...
#dotnet publish -c Release -r ios-arm64 -v d -tl:off -p:NativeLib=Static -p:PublishAotUsingRuntimePack=true "$PROJECT_PATH"
dotnet publish -c Release -r ios-arm64 -p:NativeLib=Static -p:PublishAotUsingRuntimePack=true "$PROJECT_PATH"
if [ $? -ne 0 ]; then
...
ARM64_LIB="$ARM64_DIR/AwesomeAneUtils.dylib"
X64_LIB="$X64_DIR/AwesomeAneUtils.dylib"
MACOS_UNIVERSAL_LIB="$MACOS_UNIVERSAL_DIR/AwesomeAneUtils.dylib"
...
install_name_tool -id "@loader_path/Frameworks/AwesomeAneUtils.dylib" "$MACOS_UNIVERSAL_LIB"
if [ $? -ne 0 ]; then
...
IOS_ARM64_LIB="$IOS_ARM64_DIR/AwesomeAneUtils.a"
IOS_UNIVERSAL_LIB="$IOS_UNIVERSAL_DIR/AwesomeAneUtils.a"
